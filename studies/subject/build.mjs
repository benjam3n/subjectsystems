...
const { Workbook, SpreadsheetFile } = process.argv.includes('--analyze-only') ? {} : await import('@oai/artifact-tool');
...
const model=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/model.json'),'utf8'));
const exploration=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/explorations.json'),'utf8'));
const completion=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/completions.json'),'utf8'));
const completionAnalysis=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/completion-analysis.json'),'utf8'));
const projectSubjects=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/project-subjects.json'),'utf8'));
const aboutHistory=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/about-history.json'),'utf8'));
const naming=JSON.parse(await fs.readFile(path.join(repo,'subjects/catalog.json'),'utf8'));
const nameAnalysis=JSON.parse(await fs.readFile(path.join(repo,'subjects/relations.json'),'utf8'));
const integration=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/subject-integration.json'),'utf8'));
const perspective=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/perspective-investigation.json'),'utf8'));
const perspectiveResult=JSON.parse(await fs.readFile(path.join(repo,'studies/subject/perspective-case-results.json'),'utf8'));
const study=JSON.parse(await fs.readFile(path.join(repo,'studies/system/study-systems.json'),'utf8'));
const studyResults=JSON.parse(await fs.readFile(path.join(repo,'studies/system/execution-results.json'),'utf8'));
const studyBase='https://github.com/benjam3n/subjectsystems/blob/main/';
const studySources=new Map(study.sources.map(r=>[r.id,r]));
const studyURLs=ids=>ids.map(id=>{const r=studySources.get(id);return r.url||studyBase+r.local_path;}).join('\n');
for(const r of study.studies)assert(naming.subjects.some(n=>n.name===r.subject));
...
for(let i=0;i<pn;i++){
 ...
 const text=await fs.readFile(path.join(repo,`sources/systems/${slug}.md`),'utf8');
...
const perspectiveSheets=['Perspective Distinctions','Frame Applications','Source Reconciliation','Source Associations','Approach Selection','Deviation Case'];
const studySheets=['Study Translations','Technique Contracts','Technique Admissions','Study Relations','Study Compositions','Study Reuse','Achievement Cases','Inference Cases','Study Findings','Study Sources'];
const sheetNames=['Type relation','Subject Names','Subject Index',...studySheets,...integrationSheets,...perspectiveSheets,'Naming Terms','Name Changes','Scope Relations','Expansion Families','Named Project Work','Subject Completions','Completed Types','Expression Pairs','Order Trials','Qualification Tests','Project Subjects','About History','Wording Changes','Concepts','Type Lists','Type Matrix','Concept Pairs','Reversals','Common Parents','Project Notes','Project Lists','Project Kinds','Project Pairs','Cases','Rules','Sources'];
...
function setup(name,headers,rows,widths,{height=76,table=true}={}){
 ...
 s.getRange(`A1:${titleLast}2`).merge();s.getRange('A1').values=[[name]];s.getRange(`A1:${titleLast}2`).format={fill:ink,font:{name:'Aptos Display',size:20,bold:true,color:white},rowHeight:22,verticalAlignment:'center',wrapText:true};
...
setup('Naming Terms', ['Determination term','Completion condition','Context required','Distinction from neighboring work'],nameAnalysis.operations.map(r=>[r.term,r.completion,r.required_context,r.distinction]),[26,80,68,90],{height:100});
setup('Name Changes', ['Earlier scope','Earlier name','Current names or target groups','Change relation','Reason'],nameAnalysis.name_changes.map(r=>[r.code,r.old,r.names.join('\n'),r.relation,r.reason]),[17,42,85,56,102],{height:135});
setup('Scope Relations', ['First subject','Compared subject','Relation','Separating or shared case','Consequence'],nameAnalysis.boundary_cases.map(r=>[r.a,r.b,r.relation,r.case,r.consequence]),[49,49,43,88,94],{height:112});
setup('Expansion Families', ['Target group','Defined extensions','What makes these subjects available','Boundary on further expansion'],nameAnalysis.expansion_families.map(r=>[r.target,r.names.join('\n'),r.reason,r.limit]),[33,71,82,93],{height:190});
setup('Named Project Work', ['Source family','Literal subject names','Particular matter','Proposed result','Earlier project expression','Scope fit','Standing','Pinned source profile'],nameAnalysis.project_bindings.map(r=>[r.family,r.names.join('\n')||'Exact conceptual target unresolved',r.particular_matter,r.proposed_result,r.earlier_name,r.scope_fit,r.standing,projectInfo.find(p=>p.name===r.family)?.profile||'Defining source unresolved']),[27,64,82,90,67,84,82,108],{height:139});
...
setup('Domain Translations', ['Source term','Domain view','Reading examined','Actual subjects','Translation relation','Uncovered matter or restriction','Record ID','Source URLs'],integration.translations.map(r=>[r.source_term,r.domain,r.reading,r.subjects.join('\n'),r.relation,r.residual,r.id,sourceURLs(r.source_ids)]),[43,23,78,67,34,89,13,95],{height:145});
setup('Translation Links', ['Actual subject','Domain view','Source term','Translation ID','Relation','Reading examined','Residual restriction','Source URLs'],links.map(r=>[r.subject,r.domain,r.term,r.id,r.relation,r.reading,r.residual,sourceURLs(r.source_ids)]),[53,24,43,16,34,80,88,95],{height:118});
setup('Subject Requirements', ['Subject','Requirement kind','Condition','When it matters','Declared scope','Related subject','Reason / countercase','Requirement ID','Standing'],integration.requirements.map(r=>[r.subject,r.kind,r.condition,r.timing,r.scope,r.related_subject||'No separate subject asserted',r.reason,r.id,r.standing]),[51,36,89,28,65,52,90,17,68],{height:116});
setup('Requirement Cases', ['Case','Situation','Consequence'],integration.requirement_cases.map(r=>[r.case,r.situation,r.consequence]),[37,89,105],{height:99});
const devEnd=5+integration.development.length,linkEnd=5+links.length;
setup('Subject Development', ['Subject','Importance context','Consequence to examine','Recognition / development standing','Attention measurement','World neglect','Technique IDs','Requirement IDs','Case parameters','Assessment standing','Catalog standing','Sample links','Sample domain views','Source URLs'],integration.development.map(r=>[r.subject,r.importance_context,r.importance_reason,r.recognition,r.attention,r.world_neglect,r.technique_ids.join('; ')||'No method review yet',r.requirements.join('; ')||'Further derivation open',r.case_parameters,r.assessment_standing,r.catalog_standing,null,null,sourceURLs(r.source_ids)]),[51,60,84,95,83,21,23,31,93,25,31,19,24,95],{height:135});
...
setup('Technique Evidence', ['Subject','Technique','Examined scope','Formal standing','Implementation evidence','Effectiveness evidence','Transfer limit','Evidence ID','Source URLs'],integration.techniques.map(r=>[r.subject,r.technique,r.scope,r.formal,r.implementation,r.effectiveness,r.transfer,r.id,sourceURLs(r.source_ids)]),[51,50,67,83,73,88,92,15,97],{height:138});
setup('Claim Review', ['Complaint or label','Specified case','Standing','Useful next result','Boundary on the verdict','Basis','Source URLs'],integration.claim_review.map(r=>[r.label,r.case,r.status,r.action,r.limit,r.basis,sourceURLs(r.source_ids)]),[43,88,52,93,88,40,95],{height:124});
setup('Program Compositions', ['Program','Intended result','Subject contributions','Customization needed','Next substantive result'],integration.programs.map(r=>[r.name,r.goal,r.subjects.join('\n'),r.customization,r.next_result]),[44,78,69,101,102],{height:175});
setup('Candidate Gaps', ['Subject','Observed catalog gap','Why resolving it matters','Next substantive result','Novelty standing'],integration.gaps.map(r=>[r.subject,r.gap,r.importance,r.next_result,r.novelty]),[55,89,91,98,81],{height:126});
const translationEnd=5+integration.translations.length;
setup('Domain Coverage', ['Domain view','Sample terms','Subject links','Coverage scope','Untranslated content','Next coverage result','Source URLs'],integration.coverage.map(r=>[r.domain,null,null,r.scope,r.residual,r.next_result,sourceURLs(r.source_ids)]),[25,20,20,72,121,102,95],{height:140});
...
setup('Admission Rules', ['Admission rule','What must be established'],integration.admission_rules.map(r=>[r.rule,r.test]),[47,132],{height:88});
setup('Integration Sources', ['Source ID','Source title','Role','Scope and limit','Access date','Source URL'],integration.sources.map(r=>[r.id,r.title,r.kind,r.scope,r.accessed,r.url||'Current user message and supplied attachment']),[22,86,29,127,19,108],{height:120});

setup('Study Translations', ['Subject','Particular question','Required contribution','Source family','Study ID','Origin','Standing','Admitted technique IDs','Source URLs'],study.studies.map(r=>[r.subject,r.question,r.contribution,r.family,r.id,r.origin,r.standing,r.technique_ids.join('; ')||'No complete technique admitted for this contribution',studyURLs(r.source_ids)]),[52,88,110,24,16,73,83,52,98],{height:160});
setup('Technique Contracts', ['Technique','Inputs','Operation','Output','Applicability conditions','Termination','Remaining discretion','Verification','Transfer limit','Derivation','Subjects','Implementation URL','Source URLs','ID'],study.techniques.map(r=>[r.name,r.inputs,r.operation,r.output,r.conditions,r.termination,r.remaining_discretion,r.verification,r.transfer_limit,r.derivation,r.subjects.join('\n'),studyBase+r.implementation.split(':')[0],studyURLs(r.source_ids),r.id]),[43,85,108,83,97,88,87,106,106,92,59,95,95,14],{height:205});
setup('Technique Admissions', ['Source claim or operation','Admission standing','Examined case','Retained operation or authored repair','Source URLs'],study.admission_decisions.map(r=>[r.label,r.status,r.case,r.repair,studyURLs(r.source_ids)]),[52,72,106,111,101],{height:150});
setup('Study Relations', ['First item','Relation','Second item','Case','Consequence'],study.relations.map(r=>[r.a,r.relation,r.b,r.case,r.consequence]),[34,27,33,110,110],{height:145});
setup('Study Compositions', ['System','Primary subject','Components','Actual flow','Termination','Executed case','Standing','Technique IDs','Source families','Local specification URL'],study.compositions.map(r=>[r.name,r.subject,r.components.join('\n'),r.flow,r.termination,r.case,r.standing,r.technique_ids.join('; '),r.source_families.join('; '),studyBase+r.local_path]),[42,47,66,127,98,108,99,24,33,111],{height:165});
setup('Study Reuse', ['Supplying subject','Receiving subject','Relation','Transferred content','Preserved conditions','New interpretation or operation','Actual change'],study.result_reuse.map(r=>[r.source_study,r.consumer_study,r.relation,r.transferred,r.preserved,r.added,r.actual_change]),[49,51,34,92,82,98,107],{height:144});
setup('Achievement Cases', ['Required 1','Required 2','Optional 1','Abandoned','Computed achievement','Recorded execution','Agreement','Computed activity'],studyResults.achievement_cases.map(r=>[r.input.required_1,r.input.required_2,r.input.optional_1,r.input.abandoned==='true',null,r.result.achievement,null,r.result.activity]),[22,22,22,20,28,28,19,37],{height:47});
for(let i=0;i<studyResults.achievement_cases.length;i++){const r=i+6;sh['Achievement Cases'].getRange(`E${r}`).formulas=[[`=IF(COUNTIF(A${r}:B${r},"false")>0,"not_achieved",IF(COUNTIF(A${r}:B${r},"unknown")>0,"undetermined","achieved"))`]];sh['Achievement Cases'].getRange(`G${r}`).formulas=[[`=IF(E${r}=F${r},"Matches","Changed")`]];sh['Achievement Cases'].getRange(`H${r}`).formulas=[[`=IF(D${r},"abandoned","not_marked_abandoned")`]];}
sh['Achievement Cases'].getRange('A6:C59').dataValidation={rule:{type:'list',values:['true','false','unknown']}};
sh['Achievement Cases'].getRange('G6:G59').conditionalFormats.addCustom('G6="Changed"',{fill:'#FFF0CF',font:{color:'#805700',bold:true}});
sh['Achievement Cases'].getRange('A62:C65').values=[['Achievement status','Computed count','Recorded count'],['achieved',null,null],['not_achieved',null,null],['undetermined',null,null]];
sh['Achievement Cases'].getRange('A62:C62').format={fill:teal,font:{bold:true,color:white},rowHeight:32,wrapText:true};
for(let r=63;r<=65;r++)sh['Achievement Cases'].getRange(`B${r}:C${r}`).formulas=[[`=COUNTIF($E$6:$E$59,A${r})`,`=COUNTIF($F$6:$F$59,A${r})`]];
sh['Achievement Cases'].getRange('A63:C65').format={font:{name:'Aptos',size:11,color:ink},rowHeight:30};sh['Achievement Cases'].getRange('B63:C65').setNumberFormat('#,##0');
const inferenceRows=[...studyResults.inferences,{input:{name:'Inconsistent premises',atoms:['P','Q'],premises:['P',['not','P']],conclusion:'Q'},result:studyResults.vacuous_case}];
setup('Inference Cases', ['Case','Atoms','Premises','Conclusion','Valid?','Vacuous?','Valuations checked','Premise models','Counterexamples','Scope'],inferenceRows.map(r=>[r.input.name,r.input.atoms.join('; '),JSON.stringify(r.input.premises),JSON.stringify(r.input.conclusion),r.result.valid,r.result.vacuous,r.result.valuations_checked,r.result.premise_models,JSON.stringify(r.result.counterexamples),r.result.scope]),[51,27,56,49,17,19,22,21,67,108],{height:138});
sh['Inference Cases'].getRange('G6:H12').setNumberFormat('#,##0');
setup('Study Findings', ['Proposition examined','Verdict','Actual basis','Limit'],study.claims.map(r=>[r.claim,r.verdict,r.basis,r.limit]),[68,46,118,123],{height:146});
setup('Study Sources', ['Source ID','Source title','Identity','Scope','Bytes','SHA-256','Source URL','Retained body URL'],study.sources.map(r=>[r.id,r.title,r.identity,r.scope,r.bytes??null,r.sha256||'External documentation; no local body',r.url||'Archived procedure body retained at the accompanying repository URL',r.local_path?studyBase+r.local_path:'External primary documentation']),[24,63,98,133,17,68,107,106],{height:177});
sh['Study Sources'].getRange(`E6:E${5+study.sources.length}`).setNumberFormat('#,##0');

setup('Perspective Distinctions', ['First matter or label','Compared matter or label','Relation','Separating case','Consequence'],perspective.distinctions.map(r=>[r.a,r.b,r.relation,r.case,r.consequence]),[51,49,51,100,113],{height:126});
setup('Frame Applications', ['Matter','Starting view','Transformation performed','What becomes available','What remains unsettled','Actual subjects'],perspective.frame_applications.map(r=>[r.domain,r.native,r.transformation,r.result,r.limit,r.subjects.join('\n')]),[34,70,100,96,108,72],{height:161});
const reconciliationRows=[...perspective.source_reconciliation.map(r=>[r.source_label,r.role,r.subjects.join('\n'),r.disposition,r.residual,sourceURLs(r.source_ids)]),...perspective.source_qualifications.map(r=>[r.qualification,'Defining qualification',r.subjects.join('\n'),r.force,r.consequence,sourceURLs(['WM_BRANCH'])])];
setup('Source Reconciliation', ['Source item','Record kind','Actual subjects','Disposition or logical force','Residual or consequence','Source URLs'],reconciliationRows,[43,29,64,67,103,98],{height:180});
setup('Source Associations', ['Source page','Source category','Associated label','Standing','Source URL'],perspective.source_associations.map(r=>[integrationSources.get(r.source_id).title,r.source_category,r.label,r.standing,sourceURLs([r.source_id])]),[44,23,83,108,106],{height:85});
const namingRun=perspective.naming_execution;
setup('Approach Selection', ['Candidate approach','Anticipated contribution','Examined result','Decision','Reason'],namingRun.candidates.map(r=>[r.approach,r.anticipated_contribution,r.examined_result,r.decision,r.reason]),[51,65,110,32,106],{height:160});
sh['Approach Selection'].getRange('A11:D11').values=[['Earlier name','Current name','Stable subject ID','Scope consequence']];
sh['Approach Selection'].getRange('A11:D11').format={fill:teal,font:{name:'Aptos',size:11,bold:true,color:white},rowHeight:32,wrapText:true};
sh['Approach Selection'].getRange('A12:D13').values=namingRun.result_changes.map(r=>[r.old,r.new,r.stable_id,r.effect]);
sh['Approach Selection'].getRange('A12:D13').format={font:{name:'Aptos',size:11,color:ink},wrapText:true,verticalAlignment:'top',rowHeight:120};
sh['Approach Selection'].getRange('A15:D16').merge();sh['Approach Selection'].getRange('A15').values=[[namingRun.actual_result+' '+namingRun.limits.join(' ')]];sh['Approach Selection'].getRange('A15:D16').format={font:{name:'Aptos',size:11,color:muted},wrapText:true,verticalAlignment:'center',rowHeight:57};
const payoffCase=perspective.deviation_case;
setup('Deviation Case', ['Arrangement','A payoff','B payoff','A change','B change','A versus baseline','B versus baseline','Payoff total','Dominated?'],payoffCase.outcomes.map(r=>[r.label,...r.payoffs,null,null,null,null,null,null]),[36,15,15,17,17,25,25,21,24],{height:70});
const baseRow=6+payoffCase.outcomes.findIndex(r=>r.id===payoffCase.baseline);const payoffEnd=5+payoffCase.outcomes.length;
for(let k=0;k<payoffCase.outcomes.length;k++){const r=k+6;sh['Deviation Case'].getRange(`D${r}:I${r}`).formulas=[[`=B${r}-$B$${baseRow}`,`=C${r}-$C$${baseRow}`,`=IF(D${r}>0,"Better",IF(D${r}<0,"Worse","Equal"))`,`=IF(E${r}>0,"Better",IF(E${r}<0,"Worse","Equal"))`,`=SUM(B${r}:C${r})`,`=IF(COUNTIFS($B$6:$B$${payoffEnd},">="&B${r},$C$6:$C$${payoffEnd},">="&C${r})-COUNTIFS($B$6:$B$${payoffEnd},B${r},$C$6:$C$${payoffEnd},C${r})>0,"Yes","No")`]];}
sh['Deviation Case'].getRange(`B6:E${payoffEnd}`).setNumberFormat('0;[Red](0);0');sh['Deviation Case'].getRange(`H6:H${payoffEnd}`).setNumberFormat('0;[Red](0);0');
sh['Deviation Case'].getRange('A14:I16').merge();sh['Deviation Case'].getRange('A14').values=[['Scheduled use improves both participants relative to chaotic sharing. Scheduled use and the two exclusive-use arrangements are nondominated; no arrangement attains both individual maxima. The full set has varying payoff totals. The two exclusive-use outcomes alone have a constant sum of 6. The repository calculation retains every dominance pair and the exact case assumptions.']];sh['Deviation Case'].getRange('A14:I16').format={font:{name:'Aptos',size:11,color:ink},wrapText:true,verticalAlignment:'center',rowHeight:28,fill:light};
assert.deepEqual(perspectiveResult.nondominated,['scheduled_use','exclusive_a','exclusive_b']);
assert.equal(perspectiveResult.full_domain_constant_sum,false);
assert.equal(perspectiveResult.restricted_domain_constant_sum,true);
setup('Subject Completions', ['ID','Broad label / project','Completed expression','Declared parent kinds','Role of the broad label','Exact reading / matter considered','Standing','Source','Topic-first index','Kind-first index'],completed.map(e=>[e.id,e.root,e.label,e.heads.join('; ')||'Unresolved',e.role,e.sense,e.standing,e.source,null,null]),[9,26,49,34,34,95,81,95,71,71],{height:132});
...
setup('Expression Pairs', ['Expression ID','Completed expression','Candidate kind','Result','Ground','Reason'],expressionPairRows,[14,55,24,10,48,115],{height:116});
...
setup('Completed Types', ['Broad label / project','Completed expression','Is a type of — admitted','Is not a type of — supported','Still open','Expression ID'],completed.map(e=>[e.root,e.label,null,null,null,e.id]),[27,49,67,85,92,14],{height:245});
...
setup('Order Trials', ['Form A','Form B','Explicit reading A','Explicit reading B','Kind A','Kind B','What changes','Separating or equivalence case'],completionAnalysis.orders,[32,32,64,64,27,27,78,98],{height:146});
setup('Qualification Tests', ['ID','Test','Starting expression or claim','Completed comparison','Consequence','What the test prevents'],completionAnalysis.qualification_tests,[10,44,62,88,87,100],{height:155});
...
setup('Project Subjects', ['Expression ID','Project','Completed project subject','Declared parent kinds','Exact matter','Proposed concrete contribution','Standing','Pinned source profile'],projectSubjectRows,[14,26,56,31,93,96,95,95],{height:155});
...
setup('Wording Changes', ['Period','Earlier wording / précis','Later wording / précis','Consequence for typing','Change classification','Earlier source','Later source'],completionAnalysis.wording_changes.map(r=>[...r.slice(0,5),aboutURL(r[5]),aboutURL(r[6])]),[25,78,85,107,77,115,115],{height:173});
...
setup('Concepts', ['ID','Term','Primary working sense','Alternative reading to explore','Direct parent assumptions','Standing'],conceptRows,[9,24,67,67,30,40],{height:105});
...
setup('Concept Pairs', ['Pair','A — proposed subtype','B — proposed parent','Result','Reverse','Ground','Case / rule','Reason or separating case','Overlap standing','Other relation worth preserving'],pairRows,[15,24,24,10,10,34,14,85,47,75],{height:88});
...
setup('Type Matrix', ['A → B',...terms.map(t=>t.id)],matrixRows,[27,...Array(n).fill(5)],{height:24,table:false});
...
setup('Type Lists', ['Concept','Is a type of — admitted','Is not a type of — refuted','Still open','Reading note'],terms.map(t=>[t.name,null,null,null,t.alternative]),[24,67,92,75,68],{height:240});
...
setup('Reversals', ['ID','A','B','Primary A→B / B→A','Try A as a type of B','Try B as a type of A','Common parent proposal','Consequence of the change','Discriminating case'],reversalRows,[9,22,22,24,76,76,60,67,70],{height:132});
...
setup('Common Parents', ['A','B','Most specific admitted common parents','All admitted common parents','Interpretation','A→B','B→A'],commonRows,[24,24,58,85,80,10,10],{height:74});
...
setup('Project Notes', ['ID','Project','View being classified','Source-supported kinds','Native contribution','Uses / studies / supports','Productive reinterpretation','Negative or inverse test','Source limit','Pinned source profile'],pnoteRows,[9,26,63,33,64,50,73,74,66,85],{height:140});
...
setup('Project Kinds', ['Project','Candidate kind','Result','Basis','Reason','Other relation','Pinned source profile'],pkRows,[26,24,10,48,99,62,95],{height:100});statusFormat(sh['Project Kinds'],`C6:C${pkEnd}`);
sh['Project Kinds'].getRange(`C6:C${pkEnd}`).dataValidation={rule:{type:'list',values:['Y','N','O','?','S']}};
setup('Project Lists', ['Project','Has the described kinds','Is not the stated kind in this view','Not established','Useful alternative reading'],projectInfo.map(p=>[p.name,null,null,null,p.reframe]),[28,70,94,94,74],{height:245});
...
setup('Project Pairs', ['A','B','Subtype result','Common described kinds','Shared recorded uses','What the source evidence establishes','Exact missing discrimination','A source','B source'],ppRows,[28,28,13,72,53,105,110,80,80],{height:146});statusFormat(sh['Project Pairs'],`C6:C${5+ppRows.length}`);

setup('Cases', ['ID','Constructed case','Explicit positive memberships','Explicit negative memberships','Standing'],cases.map(c=>[c.id,c.text,c.positive.join('; '),c.negative.join('; '),c.basis]),[10,105,70,92,66],{height:106});
const ruleRows=[];
...
setup('Rules', ['Rule','Kind','A / antecedent','B / consequence','Ground and limit'],ruleRows,[13,30,44,44,118],{height:91});
setup('Sources', ['Source','What it supports','Pinned profile or current analysis','Additional original-source links','Limit'],projectInfo.map(p=>[p.name,p.core,p.profile,p.sources.join('\n')||'No defining source attached',p.limit]),[28,90,110,130,82],{height:142});

setup('Type relation', ['Code','Relation','Condition'], [
 ['Y','Inclusion','Every A is B under the stated assumptions.'],
 ['N','Inclusion refutation','At least one A is not B.'],
 ['O','Overlap','A shared member and an inclusion counterexample are established.'],
 ['?','Unresolved','No sufficient inclusion or exclusion argument.'],
 ['S','Identity','The same class is compared with itself.']
], [12,30,90]);
...
const renderRanges={
 'Type relation':'A1:C10','Subject Names':'A1:E9','Subject Index':'A1:E8','Naming Terms':'A1:D9','Name Changes':'A1:E8','Scope Relations':'A1:E8','Expansion Families':'A1:D7','Named Project Work':'A1:D8','Subject Completions':'A1:F8','Completed Types':'A1:E7','Expression Pairs':'A1:F8','Order Trials':'A1:F8','Qualification Tests':'A1:F8','Project Subjects':'A1:F8','About History':'A1:F8','Wording Changes':'A1:E7','Concepts':'A1:D9','Type Lists':'A1:D7','Type Matrix':'A1:K16','Concept Pairs':'A1:H8','Reversals':'A1:F7','Common Parents':'A1:E8','Project Notes':'A1:F7','Project Lists':'A1:D7','Project Kinds':'A1:F8','Project Pairs':'A1:F7','Cases':'A1:D8','Rules':'A1:E8','Sources':'A1:E7'
};
...
Object.assign(renderRanges,{'Perspective Distinctions':'A1:D9','Frame Applications':'A1:D9','Source Reconciliation':'A1:E10','Source Associations':'A1:D9','Approach Selection':'A1:E16','Deviation Case':'A1:I16'});
Object.assign(renderRanges,{'Study Translations':'A1:D9','Technique Contracts':'A1:D8','Technique Admissions':'A1:D9','Study Relations':'A1:E9','Study Compositions':'A1:D8','Study Reuse':'A1:D9','Achievement Cases':'A1:H12','Inference Cases':'A1:H12','Study Findings':'A1:D9','Study Sources':'A1:D9'});
const changedSheets=new Set(['Type relation','Subject Names','Subject Index','Name Changes','Scope Relations',...integrationSheets,...perspectiveSheets,...studySheets]);
for(const name of sheetNames){if(process.env.SUBJECT_RENDER_ONLY&&!process.env.SUBJECT_RENDER_ONLY.split('|').includes(name))continue;if(process.env.SUBJECT_NAMES_CHANGED_ONLY==='1'&&!changedSheets.has(name))continue;const blob=await wb.render({sheetName:name,range:renderRanges[name],scale:1,format:'png'});await fs.writeFile(path.join(out,`preview-${name.replaceAll(' ','-')}.png`),new Uint8Array(await blob.arrayBuffer()));console.log(`Rendered ${name}`);}
for(const [name,range,label] of [['Subject Names','A448:E454','New-Subjects'],['Subject Requirements','A67:D73','New-Requirements'],['Domain Translations','A70:D74','New-Translations'],['Source Reconciliation','A11:E15','Source-Qualifications'],['Achievement Cases','A62:C65','Achievement-Counts']]){if(process.env.SUBJECT_RENDER_ONLY)continue;const blob=await wb.render({sheetName:name,range,scale:1,format:'png'});await fs.writeFile(path.join(out,`preview-${label}.png`),new Uint8Array(await blob.arrayBuffer()));}
const achievementCheck=await wb.inspect({kind:'table',range:"'Achievement Cases'!A62:C65",include:'values,formulas',tableMaxRows:4,tableMaxCols:3,maxChars:1800});console.log(achievementCheck.ndjson);
assert.deepEqual(sh['Achievement Cases'].getRange('B63:C65').values,[[6,6],[30,30],[18,18]]);
