...
const perspective=JSON.parse(await fs.readFile(path.join(repo,'research/type-relations/perspective-investigation.json'),'utf8'));
const perspectiveResult=JSON.parse(await fs.readFile(path.join(repo,'research/type-relations/perspective-case-results.json'),'utf8'));
...
setup('Domain Coverage','State exactly how much was translated','Counts describe the selected readings. The Working memory functional branch has a complete declared denominator; other views remain representative. No percentage of all subjects is inferred.', ['Domain view','Sample terms','Subject links','Coverage scope','Untranslated content','Next coverage result','Source URLs'],integration.coverage.map(r=>[r.domain,null,null,r.scope,r.residual,r.next_result,sourceURLs(r.source_ids)]),[25,20,20,72,121,102,95],{height:140});
...
setup('Perspective Distinctions','What each distinction changes','Comparison labels can be entry phrases, activities or claims. Their presence here does not admit them as canonical atomic subjects; Subject Names supplies the defined matters.', ['First matter or label','Compared matter or label','Relation','Separating case','Consequence'],perspective.distinctions.map(r=>[r.a,r.b,r.relation,r.case,r.consequence]),[51,49,51,100,113],{height:126});
setup('Frame Applications','Use a perspective to expose a consequence','Nine authored reconstructions. Each retains the original matter, performs a stated transformation and derives a consequence. Universal reduction and empirical effectiveness are not inferred.', ['Matter','Starting view','Transformation performed','What becomes available','What remains unsettled','Actual subjects'],perspective.frame_applications.map(r=>[r.domain,r.native,r.transformation,r.result,r.limit,r.subjects.join('\n')]),[34,70,100,96,108,72],{height:161});
...
setup('Source Reconciliation','Working memory: the complete functional branch','One construct, all four immediate functional subconstructs, and five defining qualifications, read 7 September 2026. Associated biological and paradigm labels remain separate. This is not all working-memory science.', ['Source item','Record kind','Actual subjects','Disposition or logical force','Residual or consequence','Source URLs'],reconciliationRows,[43,29,64,67,103,98],{height:180});
setup('Source Associations','Preserve associated labels without inventing subjects','97 source occurrences across four child pages. A listed molecule, cell, circuit, rhythm or paradigm does not establish a subtype, necessary mechanism, or exclusive measurement. Repeated occurrences are intentional.', ['Source page','Source category','Associated label','Standing','Source URL'],perspective.source_associations.map(r=>[integrationSources.get(r.source_id).title,r.source_category,r.label,r.standing,sourceURLs([r.source_id])]),[44,23,83,108,106],{height:85});
const namingRun=perspective.naming_execution;
setup('Approach Selection','A custom system executed on the actual naming case','Reasoning approach selection v0.1 compares three available approaches against the literal naming and scope requirements. One local result establishes execution, not general selector superiority.', ['Candidate approach','Anticipated contribution','Examined result','Decision','Reason'],namingRun.candidates.map(r=>[r.approach,r.anticipated_contribution,r.examined_result,r.decision,r.reason]),[51,65,110,32,106],{height:160});
...
setup('Deviation Case','A useful departure can have several different verdicts','Six stipulated feasible arrangements, two participants, larger payoffs preferred. Chaotic sharing is the baseline. These are mathematical inputs, not observed human preferences or scheduling effectiveness.', ['Arrangement','A payoff','B payoff','A change','B change','A versus baseline','B versus baseline','Payoff total','Dominated?'],payoffCase.outcomes.map(r=>[r.label,...r.payoffs,null,null,null,null,null,null]),[36,15,15,17,17,25,25,21,24],{height:70});
...
const notes=[
 ...
 ['Compare perspective contributions','Perspective Distinctions separates expectations, preparation, claims, value and scope. Frame Applications reconstructs nine matters and records the consequence and remaining limit.'],
 ['An actual custom system result','Approach Selection records the three candidate approaches and the executed repair: Shape representation rotation, with a distinct rotation mechanism subject and preserved identities.'],
 ['Calculate a concrete comparison','Deviation Case separates benefit relative to a baseline, dominance, a common best, and constant payoff totals. Its six arrangements and values are stipulated mathematical inputs.'],
 ['A bounded branch is reconciled','Source Reconciliation covers the Working memory construct, all four functional subconstructs and five defining qualifications. Source Associations preserves 97 associated labels without admitting placeholders. Other domain views remain representative.'],
...
const changedSheets=new Set(['Read Me','Subject Names','Subject Index','Name Changes','Scope Relations',...integrationSheets,...perspectiveSheets]);
...
for(const [name,range,label] of [['Subject Names','A390:E393','New-Subjects'],['Subject Requirements','A45:D48','New-Requirements'],['Domain Translations','A70:D74','New-Translations'],['Source Reconciliation','A11:E15','Source-Qualifications']]){const blob=await wb.render({sheetName:name,range,scale:1,format:'png'});await fs.writeFile(path.join(out,`preview-${label}.png`),new Uint8Array(await blob.arrayBuffer()));}
