...
The [integration review](research/type-relations/INTEGRATION.md) identifies three immediate results: fully reconcile one versioned source subtree, develop one actual approach-selection system with outcome evidence, and measure representation in one bounded corpus using verified aliases. The current conventional-field mappings are representative; the 39 requirement records are an examined subset.

The [literal naming inventory](subjects/scopes/NAMES.md) now supplies 384 defined subjects. Start with a consequential named matter, then develop its result. The [boundary review](cases/subject-names.md) is explicit about which pairs differ, which names were aliases, and where an exclusive hierarchy remains unresolved.
